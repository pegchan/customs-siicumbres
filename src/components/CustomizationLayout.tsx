...
type Step = 'model' | 'interiores' | 'cocina';
...
export function CustomizationLayout() {
  ...
  const handleNext = () => {
    ...
    } else if (currentStep === 'interiores') {
      setCurrentStep('cocina');
    }
      ...
      <div className="bg-white border-b">
        <div className="max-w-7xl mx-auto px-4 py-4">
          <div className="flex items-center space-x-4">
            ...
            <div className={`flex items-center ${currentStep === 'interiores' ? 'text-blue-600' : currentStep === 'cocina' ? 'text-green-600' : 'text-gray-400'}`}>
              <div className={`w-8 h-8 rounded-full flex items-center justify-center ${currentStep === 'interiores' ? 'bg-blue-600' : currentStep === 'cocina' ? 'bg-green-600' : 'bg-gray-300'} text-white font-bold`}>
                {currentStep === 'cocina' ? '✓' : '2'}
              </div>
              <span className="ml-2 font-semibold">Interiores</span>
            </div>
            <div className="flex-1 h-1 bg-gray-300"></div>
            <div className={`flex items-center ${currentStep === 'cocina' ? 'text-blue-600' : 'text-gray-400'}`}>
              <div className={`w-8 h-8 rounded-full flex items-center justify-center ${currentStep === 'cocina' ? 'bg-blue-600' : 'bg-gray-300'} text-white font-bold`}>
                3
              </div>
              <span className="ml-2 font-semibold">Cocina</span>
            </div>
          </div>
        </div>
      </div>
